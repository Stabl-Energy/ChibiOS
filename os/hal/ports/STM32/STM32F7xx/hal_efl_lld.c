...
/*===========================================================================*/
    ...
    defined(STM32F765xx) || defined(__DOXYGEN__)

/* Sector table for 2M device, single bank. */
static const flash_sector_descriptor_t efl_lld_sect1_b1[STM32_FLASH1_B1_SECTORS_TOTAL] = {
  {         0,                        32768},   /* Sector  0. */
  { 1 * 32768,                        32768},   /* Sector  1. */
  { 2 * 32768,                        32768},   /* Sector  2. */
  { 3 * 32768,                        32768},   /* Sector  3. */
  { 4 * 32768,                       131072},   /* Sector  4. */
  { 4 * 32768 + 131072,              262144},   /* Sector  5. */
  { 4 * 32768 + 131072 + 1 * 262144, 262144},   /* Sector  6. */
  { 4 * 32768 + 131072 + 2 * 262144, 262144},   /* Sector  7. */
  { 4 * 32768 + 131072 + 3 * 262144, 262144},   /* Sector  8. */
  { 4 * 32768 + 131072 + 4 * 262144, 262144},   /* Sector  9. */
  { 4 * 32768 + 131072 + 5 * 262144, 262144},   /* Sector 10. */
  { 4 * 32768 + 131072 + 6 * 262144, 262144},   /* Sector 11. */
};

/* Sector table for 2M device, dual bank. */
static const flash_sector_descriptor_t efl_lld_sect1_b2[STM32_FLASH1_B2_SECTORS_TOTAL] = {
  {         0,                            16384},   /* Sector  0. */
...
/* Sector table for 1M device, single bank. */
static const flash_sector_descriptor_t efl_lld_sect2_b1[STM32_FLASH2_B1_SECTORS_TOTAL] = {
  {         0,                        32768},   /* Sector  0. */
  { 1 * 32768,                        32768},   /* Sector  1. */
  { 2 * 32768,                        32768},   /* Sector  2. */
  { 3 * 32768,                        32768},   /* Sector  3. */
...
/* Sector table for 1M device, dual bank */
static const flash_sector_descriptor_t efl_lld_sect2_b2[STM32_FLASH2_B2_SECTORS_TOTAL] = {
  {         0,                            16384},   /* Sector  0. */
  { 1 * 16384,                            16384},   /* Sector  1. */
  { 2 * 16384,                            16384},   /* Sector  2. */
  { 3 * 16384,                            16384},   /* Sector  3. */
...
#elif defined(STM32F756xx) || defined(STM32F746xx) ||                         \
      defined(STM32F745xx)
...
/* Sector table for 512k device, single bank. */
static const flash_sector_descriptor_t efl_lld_sect2_b1[STM32_FLASH2_B1_SECTORS_TOTAL] = {
  {         0,                        32768},   /* Sector  0. */
  { 1 * 32768,                        32768},   /* Sector  1. */
  { 2 * 32768,                        32768},   /* Sector  2. */
...
/* Sector table for 256K device, single bank. */
static const flash_sector_descriptor_t efl_lld_sect2_b1[STM32_FLASH2_B1_SECTORS_TOTAL] = {
  {         0,                        16384},   /* Sector  0. */
  { 1 * 16384,                        16384},   /* Sector  1. */
  { 2 * 16384,                        16384},   /* Sector  2. */
...
/* Sector table for 64K device, single bank. */
static const flash_sector_descriptor_t efl_lld_sect1_b1[STM32_FLASH1_B1_SECTORS_TOTAL] = {
  {         0,                        16384},   /* Sector  0. */
  { 1 * 16384,                        16384},   /* Sector  1. */
  { 2 * 16384,                        16384},   /* Sector  2. */
...
static const flash_descriptor_t efl_lld_size1[STM32_FLASH_NUMBER_OF_BANKS] = {
      { /* Single bank organisation. */
       ...
       .sectors_count     = STM32_FLASH1_B1_SECTORS_TOTAL,
       .sectors           = efl_lld_sect1_b1,
       .sectors_size      = 0,
      ...
      { /* Dual bank organisation. */
       .attributes        = FLASH_ATTR_ERASED_IS_ONE |
                            FLASH_ATTR_MEMORY_MAPPED,
       .page_size         = STM32_FLASH_LINE_SIZE,
       .sectors_count     = STM32_FLASH1_B2_SECTORS_TOTAL,
       .sectors           = efl_lld_sect1_b2,
       .sectors_size      = 0,
       .address           = (uint8_t *)FLASH_BASE,
       .size              = STM32_FLASH1_SIZE * STM32_FLASH_SIZE_SCALE
      }
...
#ifdef STM32_FLASH2_SIZE
static const flash_descriptor_t efl_lld_size2[STM32_FLASH_NUMBER_OF_BANKS] = {
      { /* Single bank organisation. */
       .attributes        = FLASH_ATTR_ERASED_IS_ONE |
                            FLASH_ATTR_MEMORY_MAPPED,
       .page_size         = STM32_FLASH_LINE_SIZE,
       .sectors_count     = STM32_FLASH2_B1_SECTORS_TOTAL,
       .sectors           = efl_lld_sect2_b1,
       .sectors_size      = 0,
       .address           = (uint8_t *)FLASH_BASE,
      ...
      { /* Dual bank organisation. */
       .attributes        = FLASH_ATTR_ERASED_IS_ONE |
                            FLASH_ATTR_MEMORY_MAPPED,
       .page_size         = STM32_FLASH_LINE_SIZE,
       .sectors_count     = STM32_FLASH2_B2_SECTORS_TOTAL,
       .sectors           = efl_lld_sect2_b2,
       .sectors_size      = 0,
       .address           = (uint8_t *)FLASH_BASE,
       .size              = STM32_FLASH2_SIZE * STM32_FLASH_SIZE_SCALE
      }
...
void efl_lld_start(EFlashDriver *eflp) {
  ...
  if (eflp->flash->CR & FLASH_CR_LOCK)
...
flash_error_t efl_lld_program(void *instance, flash_offset_t offset,
  ...
  while (n > 0U) {
    ...
    case 1:
      *(volatile uint8_t*)address = line.b[0];
      break;

    case 2:
      *(volatile uint16_t*)address = line.h[0];
      break;
     ...
     * This will force the CPU to respect the sequence of instructions
     * (no optimization).*/
    __DSB();
...
/* Only allow mass erase on dual bank devices */
#if defined(FLASH_CR_MER2)
  /* Dual bank mode has to be activated. */
...
flash_error_t efl_lld_start_erase_sector(void *instance,
  ...
  /* On dual bank configuration, MSB selects the bank and sector number starts
   * counting from 0 again. */
  if (stm32_flash_dual_bank(devp) && sector >= (bank->sectors_count / 2)) {
